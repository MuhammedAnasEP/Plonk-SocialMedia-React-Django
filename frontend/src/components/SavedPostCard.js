...
import { comments, getcomments, getlike, getpost, getsavedpost, like, savepost, getuser, editpost, deletepost, deletecomment, savedpost } from "../Constants/Constants"
            ...
            <div>                
                <Card className="">                  
                    <div className="flex gap-3 mb-2">
                        ...
                        <div className="grow">
                            <Link to='/profile'><p><span className="font-semibold cursor-pointer hover:underline">{data.post.user.username}</span> Shared a <span className="text-socialBlue">post</span></p></Link>
                    ...
                    <div className="flex justify-between items-center mt-2">
                        <div className="flex gap-8">
                            {
                                likes && likes.map((data)=>(
                                    data.user === user.user_id && data.id === data.post ?
                            ...
                            {likes && likes.length === 0 && <button onClick={()=>{Like(user.user_id,data.post.id)}} className="flex gap-2 items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
                                        </svg>
                                    </button>}                                                
                        ...
                        <div className="">
                            {savedposts?.map((datas)=>(
                                data.user.id === user.user_id && datas.post.id === data.post.id ? 
                    ...
                    <div className="flex gap-2 mt-3">
                        ...
                        <button onClick={()=>{addComment(user.user_id, data.post.id)}} className="top-0 text-blue-500 font-bold">Post</button>
                    </div>

                    <span onClick={()=>{toggleCommentModal(data.post.id, data.post.image)}} className="text-sm text-gray-500 cursor-pointer">Show all comments</span>
